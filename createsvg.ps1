Get-Item -Path '.\*.csv' | ForEach-Object {
    $CSVFile = $_.FullName
    $SVGFile = $CSVFile -replace '.csv', '.svg'

    $MaximumValues = 365
    $LineHeight = 25

    $Values = Import-Csv -Delimiter ',' -Path $CSVFile -Header 'Date', 'Value' | Select-Object -Property 'Value' -Last $MaximumValues
